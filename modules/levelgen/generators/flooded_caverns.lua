...
--- @class FloodedCavern : Generator
local FloodedCavern = prism.levelgen.Generator:extend("FloodedCavern")
local DEBUG = false

--- @param generatorInfo GeneratorInfo
--- @param player Actor
function FloodedCavern:generate(generatorInfo, player, rng)
	...
	for _ = 4, 8 do
		room = RoomManager.roomGraph.rooms[rng:random(1, #RoomManager.roomGraph.rooms)]
		prism.decorators.FloodedRoomDecorator.tryDecorate(generatorInfo, rng, builder, room)
	end
...
function FloodedCavern:addRandomPits(builder, width, height, rng)
